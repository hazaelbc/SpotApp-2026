...
export const BarraLateral = ({ children, className = "", compact = false }) => {
    return(
        <aside className={`${compact ? 'w-full' : 'w-64'} flex ${className}`}>
            <nav className={`flex-1 flex flex-col ${compact ? 'p-0' : 'p-4'} space-y-2`}>
                {/* Sección de navegación */}
                <div className="space-y-1">
                    <button className={`w-full flex items-center gap-3 ${compact ? 'pl-0 py-3' : 'py-2'} text-sm text-gray-700 dark:text-[var(--text-primary)] hover:bg-gray-200 dark:hover:bg-[var(--bg-tertiary)] rounded-lg transition-colors duration-200 min-w-0`}>
